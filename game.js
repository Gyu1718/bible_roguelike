...
function chaptersPage(save) {
  ...
  return `
    <section class="home-hero">
      ...
      <div class="save-summary"><span>회차 ${save.totalRuns || 0}</span><span>엔딩 ${endingsCount}</span><span>증언 유형 ${save.testimonyTypesSeen?.length || 0}</span></div>
    </section>
    <section class="chapter-grid">
      <button class="chapter-card ${cleared ? "cleared" : "active"}" onclick="restart()"><span>${cleared ? "클리어" : "진행 가능"}</span><strong>1장 벽돌과 바다</strong><em>${cleared ? `발견 엔딩 ${endingsCount}개` : "홍해를 건너기 위한 믿음과 선택의 순간"}</em></button>
...
function recordsPage(save) {
  ...
  return `
    <section class="home-hero archive-hero">
      <p class="eyebrow">RECORDS</p><h2>기록 보관소</h2><p>당신이 지나온 엔딩과 마지막 증언이 이곳에 남습니다.</p>
...
function witnessesPage(save) {
  ...
  return `<section class="home-hero archive-hero"><p class="eyebrow">WITNESSES</p><h2>증언 유형</h2><p>플레이 중 드러난 숨은 신앙의 흔적입니다. 게임 중에는 보이지 않고 엔딩에서만 드러납니다.</p></section><section class="archive-grid">${allTestimonyTypes().map((type) => { const unlocked = seen.includes(type); return `<article class="archive-card ${unlocked ? "" : "locked"}"><span>${unlocked ? "발견" : "미발견"}</span><strong>${unlocked ? type : "???"}</strong><em>${unlocked ? "엔딩에서 확인한 증언 유형" : "다른 선택으로 발견 가능"}</em></article>`; }).join("")}</section>`;
}
...
function homeContent(save) {
  const pages = { chapters: () => chaptersPage(save), witnesses: () => witnessesPage(save), records: () => recordsPage(save), artifacts: () => artifactsPage(save), achievements: () => achievementsPage(save), settings: () => settingsPage(save) };
  return (pages[state.activeMenu] || pages.chapters)();
}

function homeScreen() { const save = getSaveData(); return `<main class="home-screen">${menuNav()}<section class="home-content">${homeContent(save)}</section></main>`; }
function playScreen() { const scene = scenes[state.index]; return `<main class="layout play-screen">${sceneArtSection(scene, "ILLUSTRATION")}<section class="card story-card"><p class="eyebrow">벽돌과 바다 · ${state.index + 1}/${scenes.length}</p><h2>${scene.title}</h2><p>${scene.text}</p><div class="choices">${scene.choices.map(choiceButton).join("")}</div></section>${side()}</main>`; }
function rollScreen() { const scene = scenes[state.index]; const choice = state.pendingChoice; const roll = state.roll; const modText = roll.modifier === 0 ? "보정 없음" : `보정 ${roll.modifier > 0 ? "+" : ""}${roll.modifier}`; return `<main class="layout play-screen roll-screen">${sceneArtSection(scene, "2D6 CHECK")}<section class="card story-card"><p class="eyebrow">판정</p><h2>${choice.label}</h2><p>위험한 선택이다. 주사위 두 개를 굴려 목표값 이상이면 성공한다.</p><div class="meta"><div><span>주사위</span><strong>${roll.d1} + ${roll.d2}</strong></div><div><span>${modText}</span><strong>총 ${roll.score}</strong></div><div><span>목표값</span><strong>${roll.target} 이상</strong></div><div><span>결과</span><strong>${roll.success ? "성공" : "실패"}</strong></div></div><button class="primary" onclick="resolveRoll()">결과 확인</button></section>${side()}</main>`; }
function feedbackScreen() { const scene = scenes[state.index]; const feedback = state.feedback; return `<main class="layout play-screen feedback-screen">${sceneArtSection(scene, "선택의 결과")}<section class="card story-card"><p class="eyebrow">선택의 결과</p><h2>${feedback.title}</h2><p>${feedback.text}</p><div class="meta"><div><span>생존</span><strong>${state.stats.endurance}</strong></div><div><span>공포</span><strong>${state.stats.panic}</strong></div><div><span>증언</span><strong>${state.stats.witness}</strong></div><div><span>분기</span><strong>${feedback.next === "bad" ? "배드엔딩" : "계속 진행"}</strong></div></div><button class="primary" onclick="proceed()">${feedback.next === "bad" ? "배드엔딩 확인" : "계속"}</button></section>${side()}</main>`; }
function endingScreen() { const ending = state.ending; const type = testimonyType(); return `<main class="ending-screen"><section class="ending-panel"><p class="eyebrow">엔딩</p><h2>${ending.title}</h2><div class="type-ribbon">당신의 증언 유형: ${type.title}</div><p>${ending.text}</p><p>${type.text}</p><div class="meta"><div><span>생존</span><strong>${state.stats.endurance}</strong></div><div><span>공포</span><strong>${state.stats.panic}</strong></div><div><span>증언</span><strong>${state.stats.witness}</strong></div><div><span>기록</span><strong>${state.log.length}개</strong></div></div><div class="row"><button class="primary" onclick="restart()">다시 도전</button><button class="secondary" onclick="goHome('chapters')">처음으로</button></div></section></main>`; }

function render() {
  const root = document.getElementById("root");
  const body = state.screen === "home" ? homeScreen() : state.screen === "roll" ? rollScreen() : state.screen === "feedback" ? feedbackScreen() : state.screen === "ending" ? endingScreen() : playScreen();
  root.innerHTML = `<div class="app screen-${state.screen}"><header class="top"><div><p class="eyebrow">BIBLICAL WITNESS ROGUELIKE</p><h1>이름 없는 증인들</h1><p>성경 세계관 선택지형 로그라이크</p></div><button class="secondary" onclick="restart()">새 회차</button></header>${body}</div>`;
}
